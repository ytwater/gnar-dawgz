/**
 * Generated by orval v8.1.0 🍺
 * Do not edit manually.
 * WAHA - WhatsApp HTTP API
 * <b>WhatsApp HTTP API</b> that you can run in a click!<br/><a href="/dashboard"><b>📊 Dashboard</b></a><br/><br/>Learn more:<ul><li><a href="https://waha.devlike.pro/" target="_blank">Documentation</a></li><li><a href="https://waha.devlike.pro/docs/how-to/engines/#features" target="_blank">Supported features in engines</a></li><li><a href="https://github.com/devlikeapro/waha" target="_blank">GitHub - WAHA Core</a></li><li><a href="https://github.com/devlikeapro/waha-plus" target="_blank">GitHub - WAHA Plus</a></li></ul><p>Support the project and get WAHA Plus version!</p><ul><li><a href="https://waha.devlike.pro/docs/how-to/plus-version/" target="_blank">WAHA Plus</a></li><li><a href="https://patreon.com/wa_http_api/" target="_blank">Patreon</a></li><li><a href="https://boosty.to/wa-http-api/" target="_blank">Boosty</a></li><li><a href="https://portal.devlike.pro/" target="_blank">Patron Portal</a></li></ul>
 * OpenAPI spec version: 2026.1.5
 */
import type { ChattingControllerGetMessagesFilterAck } from "./chattingControllerGetMessagesFilterAck";
import type { ChattingControllerGetMessagesSortBy } from "./chattingControllerGetMessagesSortBy";
import type { ChattingControllerGetMessagesSortOrder } from "./chattingControllerGetMessagesSortOrder";

export type ChattingControllerGetMessagesParams = {
	/**
	 * Sort by field
	 */
	sortBy?: ChattingControllerGetMessagesSortBy;
	/**
	 * Sort order - <b>desc</b>ending (Z => A, New first) or <b>asc</b>ending (A => Z, Old first)
	 */
	sortOrder?: ChattingControllerGetMessagesSortOrder;
	/**
	 * Download media for messages
	 */
	downloadMedia?: boolean;
	chatId: string;
	session?: string;
	limit?: number;
	offset?: number;
	/**
	 * Filter messages before this timestamp (inclusive)
	 */
	"filter.timestamp.lte"?: number;
	/**
	 * Filter messages after this timestamp (inclusive)
	 */
	"filter.timestamp.gte"?: number;
	/**
	 * From me filter (by default shows all messages)
	 */
	"filter.fromMe"?: boolean;
	/**
	 * Filter messages by acknowledgment status
	 */
	"filter.ack"?: ChattingControllerGetMessagesFilterAck;
};
